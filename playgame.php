...
try {
    ...
    $username = htmlspecialchars($row['name']); // Fetch the username for display
    ...
    if (isset($_POST["enroll_subject"])) {
        ...
        if ($subject) {
            ...
            $enroll_stmt = $conn->prepare("INSERT INTO student_subjects (student_id, subject_id) VALUES (:student_id, :subject_id)");
            $enroll_stmt->execute(['student_id' => $id, 'subject_id' => $subject_id]);
            echo "<script>alert('Enrolled in subject: " . htmlspecialchars($subject['name']) . "');</script>";
        } else {
