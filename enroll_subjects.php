...
if (isset($_POST["enroll_subject"])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT * FROM subjects WHERE code = :code");
        $stmt->execute(['code' => $subject_code]);
        $subject = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($subject) {
            // Check if already enrolled
            $stmt = $conn->prepare("SELECT * FROM enrollments WHERE student_id = :student_id AND subject_id = :subject_id");
            $stmt->execute(['student_id' => $student_id, 'subject_id' => $subject['id']]);
            $enrollment = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if (!$enrollment) {
                // Enroll in the subject
                $stmt = $conn->prepare("INSERT INTO enrollments (student_id, subject_id) VALUES (:student_id, :subject_id)");
                $stmt->execute(['student_id' => $student_id, 'subject_id' => $subject['id']]);
                echo "<script>alert('Successfully enrolled in subject: " . htmlspecialchars($subject['name']) . "');</script>";
            } else {
                echo "<script>alert('Already enrolled in this subject');</script>";
            }
        } else {
        ...
        <section class="hero-without-image-2">
            <div class="container-2">
                <div class="hero-wrapper-two-2">
                    <h1 class="heading-2">Enroll in Subject</h1>
                    <form method="post" action="">
                        <label for="subject_code">Subject Code:</label>
                        <input type="text" id="subject_code" name="subject_code" required>
                        <button type="submit" name="enroll_subject">Enroll</button>
                    </form>
                </div>
